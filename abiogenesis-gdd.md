...
- **Observation log:** salient events recorded era by era (blooms, collapses, extinctions, notable adjacencies). **[PROPOSED]** each entry carries a *clean vs confounded* indicator, surfacing the observation weight below without stating the formula — teaching by reinforcement that isolating an experiment pays. **[DECIDED, task 181 gap resolution]** Separately from that still-proposed indicator, every entry also carries an **outcome/valence** (positive — birth, speciation; negative — death, extinction; neutral — discovery, confirmation, objective progress), read by its marker color. This is a *presentation-layer classification of which event produced the entry*, fixed per event kind at the point the entry is pushed (extinction and a player-placed organism's death are negative; speciation and an era's birth tally are positive; terrain reveal, objective advance, and matrix confirmation are neutral) — not a new simulation signal, and not the same axis as the clean/confounded indicator above, which is about evidence quality on confirmation-type entries specifically, not about outcome sign.
