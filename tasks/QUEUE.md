...
| `IN_PROGRESS` | 191 | "How to play" presentation: reuse notebook chrome (`section_header`/`hairline`), bullet content instead of dense paragraphs, responsive panel height instead of fixed pixel constants, trim the mandatory intro to a 3-section primer (full guide stays opt-in from the main menu). **Amendment 1 (2026-09-02)** fixed width containment/grouping. **Amendment 2 (2026-09-02)**: screenshot review found wrapped bullet lines have no hanging indent + two metabolism bullets each state two facts; implemented (`ui.horizontal_top` + `.wrap()` per-bullet, metabolism lines split, bullet spacing added) but still owed a user live-check before `ACCEPTED` — see task file | `VISUAL_STYLE_GUIDE.md` §2/§6 — [191](191-how-to-play-presentation.md) |
