...
| `IN_PROGRESS` | 171 | Causal-legibility playtest gate: bot-vs-bot necessity check (done, pass) + human playtest protocol (written, **not yet run**), gates Phase 3. Onboarding sync (190) accepted — human playtest unblocked, still awaiting a real playtester | GDD §5.8/§5.9 — [171](171-causal-legibility-playtest-gate.md), [results](171-results.md) |
| `ACCEPTED` | 190 | Sync onboarding content (in-game "How to play" panel + `player_guide.md`) to current mechanics — missing 4th metabolism, 4 objective kinds, Chronicle, Splice's confirmed-trait restriction, continuous-advance key. Build/clippy/fmt/test clean; live-verified 2026-09-02. Reviewed and accepted 2026-09-02 | `player_guide.md`, `src/text.rs` — [190](190-onboarding-content-sync.md) |
| `IN_PROGRESS` | 191 | "How to play" presentation: reuse notebook chrome (`section_header`/`hairline`), bullet content instead of dense paragraphs, responsive panel height instead of fixed pixel constants, trim the mandatory intro to a 3-section primer (full guide stays opt-in from the main menu). **Amendment 1** fixed width containment/grouping. **Amendment 2** fixed hanging indent + split metabolism bullets — confirmed working by user screenshot. **Amendment 3 (2026-09-02)**: wider content column, bold lead-term on "Term: description" bullets (Controls, Objectives) with dimmed description, split the two remaining multi-item bullets (Space/Shift+Space, the original 3 objective kinds) for consistency — see task file | `VISUAL_STYLE_GUIDE.md` §2/§6 — [191](191-how-to-play-presentation.md) |
| `QUEUED` | 192 | Key the starting-species unlock off cumulative understanding (relationships confirmed), not worlds cleared — GDD §10's own rule names "objectives cleared" as exactly the wrong proxy. Depends on 158's cumulative aggregates | `abiogenesis-gdd.md` §10 — [192](192-unlock-trigger-cumulative-understanding.md) |
...
**Unblocked by task 190 (2026-09-02):** the in-game "How to play" panel and
...
190 was reviewed and recorded `ACCEPTED` on 2026-09-02; 171's human
playtest half is no longer blocked on stale onboarding.
...
| `[x]` | 181 | Notebook chrome fidelity: neutral relationship-graph nodes (amber stroke, not tag-colored), Catalog block-pattern icons. Code lands 2026-08-30, build/clippy/test clean; user screenshot confirmed nodes/Catalog icons render correctly and surfaced a real tiling bug in 180's `asterisk_mask` (fixed, see 180's own file); live-check deliberately skipped per user instruction (see task file). Observation-log marker AC left unfixed here (no valence signal in `LogEntry` to key off, not invented) — resolved by GDD §7 decision and spun off to task 189 | `population-model-aesthetic` — [181](done/181-notebook-chrome-fidelity.md) |
...
| `[x]` | 186 | Notebook/inspect-card monospace gap + Seed Palette and Chronicle species-color leaks (found via user screenshot audit, missed by 180-185); live-verified after 187/188's follow-ups | `VISUAL_STYLE_GUIDE.md` — [186](done/186-notebook-inspect-seedlist-chrome-gaps.md) |
| `[x]` | 187 | Seed Palette scroll height (2-line row budgeted as 1) + notebook text clipping (missing `.wrap()` in 3 more spots), both surfaced by user screenshots of 186's own result; live-verified after 188's follow-up | `VISUAL_STYLE_GUIDE.md` — [187](done/187-species-list-height-notebook-text-wrap.md) |
| `[x]` | 188 | **Real gameplay bug**: continuous-advance never reset between worlds (leaked `space`→era behavior); plus notebook outer-scroll gap and pause/confirmation dialog overlap; live-verified 2026-08-31 | user report — [188](done/188-continuous-advance-leak-notebook-scroll-menu-overlap.md) |
| `QUEUED` | 189 | Observation-log markers: outcome color (positive/negative/neutral by event kind), not species color — resolves task 181's flagged gap via a GDD §7 decision (presentation-layer classification, no new sim state) | `abiogenesis-gdd.md` §7 — [189](189-observation-log-outcome-color.md) |
