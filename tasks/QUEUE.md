...
| `IN_PROGRESS` | 191 | "How to play" presentation: reuse notebook chrome (`section_header`/`hairline`), bullet content instead of dense paragraphs, responsive panel height instead of fixed pixel constants, trim the mandatory intro to a 3-section primer (full guide stays opt-in from the main menu). **Amended 2026-09-02**: live check found it still "crowded and mixed" — centered menu column vs. full-width left-aligned guide content, needs a contained width + more grouping separation, see the task file's own amendment | `VISUAL_STYLE_GUIDE.md` §2/§6 — [191](191-how-to-play-presentation.md) |
