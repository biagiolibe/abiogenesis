...
- `[ ]` 069 — Multi-octave terrain noise (macro-continents + small islands): follow-up raised directly by the user after reviewing 068's rendering, not in the original redesign doc. Task 066's elevation field is single-scale (all waves share one frequency range), so a world never shows both a large landmass and small separate islands at once. Splits `terrain_waves`/`terrain_elevation` into a low-frequency "continent" band plus a smaller-weight higher-frequency "island/detail" band, both new `TerrainConfig` fields, same derived RNG stream → [069](tasks/069-multi-octave-terrain-noise.md)
...
*Last updated: 2026-08-09 (task 071 complete: `cell_color`'s residue branch now ignores task 060's ambient trickle floor, fixing a second terrain-color regression the user caught in the same screenshot thread as 070; 069 — multi-octave noise — still pending)*
