...
- `[x]` 134b — Make the two bot policies competent (they reached dozens of organisms where a greedy diagnostic reached thousands, so they measured a strategy nobody would play) and re-record the baseline 136 is verified against → [134b](tasks/done/134b-competent-bot-policies-and-rebaseline.md)
- `[x]` 136b — Evidence per distinct observation, not per tick: `AdjacencyObserved` now fires only on adjacency onset (`SimWorld::adjacency_exposure`), not every tick it persists — measured 12.2M events → 204 on the same diagnostic, confirmation now flat under idling instead of doubling with elapsed time → [136b](tasks/done/136b-evidence-per-distinct-observation.md)
- `[x]` 135 — Three-level time scale Pulse → Season → Era: season as the unit of decision, action budget per season, world era budget lowered, `selection_pressure_threshold` retuned → [135](tasks/done/135-three-level-time-scale.md)
- `[x]` 136 — Make the hidden matrix necessary: `interaction_scale` + proportional retune of all four metabolisms (gains settled at `÷1.43`, not the doc's `÷2.5`, after `tests/balance.rs` caught the doc's proposal dying to diffusion drift); `crowd_factor` scoped to cross-species neighbours only → [136](tasks/done/136-matrix-necessary-balance.md)
...
sidebar list width, `[x]` 174 toxicity label/tint + population tooltip clarity, `[x]` 175
...
untracked `species_color` swatch violation) `[x]`. 182 shipped the shared
...
coupled and must be tuned together, not in isolation. 084 was scoped but
explicitly blocked; unblocked 2026-08-29 — see its file.
...
- `[x]` 084 — Guaranteed "first light" relation in world 0's matrix — unblocked and implemented 2026-08-29: applies to every `world_index == 0` instead of gating on the "Meta-progression persistence" proposal (§1), per user decision following the first human playtest's engagement findings (`playtest_outcome.md`). Implementation found a real trade-off the AC hadn't anticipated: forcing any pair among palette-carried tags risks breaking `draw_species_tags`'s net-zero self-interaction invariant (task 048/088) when one species monopolizes the palette's tags (~3% of seeds) — resolved with a two-tier candidate search (safe cross-species pair preferred; falls back to pairing a palette tag with a currently-unused active tag) → [084](tasks/done/084-first-light-guaranteed-relation-world0.md)
