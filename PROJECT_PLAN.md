...
- `[?]` **1.A — Instant visual "spark" on interaction**: fires only on the first-ever observation of a given relation (not on every `interaction_delta`, to avoid visual noise on dense grids), so the hidden matrix reads as an observed event instead of a deduced one. Presentation-only, no balance change — likely highest impact-to-cost of the whole set. Visually inspired by the existing seed-placement ring (`render.rs`, `placement_indicator` — `PlacementIndicator` resource, fixed-radius alpha-fade over 0.6s): kept simple for now, same fixed radius and alpha-only fade, no radius-shrink animation; the one real generalization needed is multi-instance support (today the resource is a single slot, but multiple relations can be confirmed in the same tick). Shows in both Detail (on the exact cell) and Overview (aggregated on the involved cluster/blob).
- `[?]` **1.B — Guaranteed "first light" in world 0's worldgen**: no auto-placed species (stays banned), but a world-0-only generation constraint guaranteeing a strong (`±2`) matrix relation is reachable and visible within era 1-2, near the player's first seed. **Blocked** on the "Meta-progression persistence" proposal (§ "Open questions from the GDD (§14)"): without real persistence there's no way to distinguish a player's true first-ever world from any restart, so shipping this today would guarantee the relation on *every* run, not just the first one — undermining unpredictability for returning players.
- `[?]` **1.C — Newborn incubation**: an organism born this era can't itself reproduce until the next era — slows the initial swarm without touching `repro_threshold` or existing energy economy. **Decided as a permanent system rule**, not scoped to world 0/onboarding like 1.D — promoting this to a task requires first re-validating the existing balance/determinism tests (task 009, carrying-capacity GDD §13), since it affects population dynamics for the whole game, not just the opening swarm. Tune jointly with 1.D, not in isolation: once onboarding eras are short, the era boundary from 1.C becomes the dominant pacing constraint, not the energy threshold 1.D was meant to modulate.
