...
Return APPROVE, CHANGES_REQUESTED, or BLOCKED. End the handoff with the fields from `docs/COMPLETION_REPORT_TEMPLATE.md` plus the verdict. After APPROVE only, update exactly the task file's and `tasks/QUEUE.md`'s status to ACCEPTED and commit `docs: accept <TASK-ID>` — this project has no forge/PR step, so acceptance is a direct, separate commit from the implementation. The reviewer-integrator identity must be distinct from whoever implemented the task; never self-approve. Never modify implementation code as part of a review — file a new task instead.
